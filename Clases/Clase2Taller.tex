...
Sea $U(\vec{r})$ un potencial que depende solo de la posición, y sea $\vec{r}(t)$ una trayectoria definida en el espacio tridimensional.

\begin{enumerate}[label=\alph*)]
    \item Escriba la derivada total $dU/dt$ usando la regla de la cadena multivariada en términos de las derivadas parciales $\partial U/\partial x, \partial U/\partial y, \partial U/\partial z$ y las componentes de la velocidad $\dot{x}, \dot{y}, \dot{z}$.
    
    \item Reescriba el resultado en notación vectorial y demuestre que 
    $dU/dt = \nabla U(\vec{r}(t)) \cdot \dot{\vec{r}}(t)$.
\end{enumerate}

\section{Solución Problema \# 4}

Este problema trata sobre el cálculo de la **tasa de cambio temporal** de una función escalar (potencial) a lo largo de la trayectoria de una partícula. Se busca aplicar la **regla de la cadena multivariada** para descomponer el cambio total en términos de las variaciones espaciales del potencial y el vector velocidad de la partícula.

\subsection{Parte (a): Aplicación de la regla de la cadena}

Cuando una partícula se mueve siguiendo una trayectoria 
$\vec{r}(t) = (x(t), y(t), z(t))$, el potencial $U$ cambia en el tiempo debido al cambio de posición de la partícula. Matemáticamente, $U$ es una función compuesta:

\[
U = U(x(t), y(t), z(t)).
\]

De acuerdo con la regla de la cadena para funciones de varias variables, la derivada total respecto al tiempo es
...
+
\frac{\partial U}{\partial z}\frac{dz}{dt}.
\]

Utilizando la notación de punto para las velocidades ($\dot{x}, \dot{y}, \dot{z}$):
...
+
\frac{\partial U}{\partial z}\dot{z}.
\]

Como el potencial depende **solo de la posición**, se tiene

\[
\frac{\partial U}{\partial t}=0.
\]

\subsection{Parte (b): Demostración en notación vectorial}

Identificamos dos entidades vectoriales:

\textbf{Gradiente del potencial}
...
El producto escalar entre estos vectores es
...
\begin{enumerate}
\item \textbf{Conservación de la energía:} Esta relación es clave para demostrar la conservación de la energía mecánica en sistemas conservativos.

\item \textbf{Dinámica de Hamilton:} Permite describir la evolución de sistemas físicos en el espacio de fase usando potenciales escalares.

\item \textbf{Teoría de perturbaciones:} Facilita calcular cómo cambia la energía de una órbita cuando actúan fuerzas adicionales.
\end{enumerate}
