...
\begin{titlepage}
  ...
  \includegraphics[width=0.24\textwidth]{imgCRTBP/logo-udea.png}\\[0.6cm]
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Explorar, de forma cualitativa, un escenario de captura balística temporal cerca del punto de Lagrange \(L_1\), usando la energía de Hilda como referencia.
\end{enumerate}
...
\subsection{Aproximación de Euler para el punto \texorpdfstring{\(L_1\)}{L1}}
Para \(\mu\ll1\), la posición del punto colineal \(L_1\) (entre el secundario y el primario) puede aproximarse a primer orden mediante la fórmula de Euler,
\begin{equation}
x_{L_1}\approx (1-\alpha)-\left(\frac{\alpha}{3}\right)^{1/3},
\end{equation}
válida solo como primera aproximación y no como solución exacta de la condición de equilibrio \(\nabla V_{mod}=0\) del CRTBP. Esta aproximación se utiliza únicamente con fines exploratorios en la Sección~\ref{sec:captura}.
...
\subsection{Experimento exploratorio de captura balística cerca de \texorpdfstring{\(L_1\)}{L1}}
\label{sec:metodologia_captura}
Como complemento cualitativo al análisis anterior, se diseñó un experimento numérico para ilustrar el concepto de \emph{captura balística temporal}. El procedimiento fue el siguiente:

\begin{enumerate}
  \item Se calculó \(x_{L_1}\) mediante la aproximación de Euler y se evaluó allí la constante de Jacobi, \(C_J^{(L_1)}=2\,\Omega(\vec r_{L_1})\), con \(\Omega\) el potencial efectivo.
  \item Se tomó como referencia energética la constante de Jacobi real de 153 Hilda, \(C_J^{(\text{Hilda})}\), calculada a partir de su vector de estado ya obtenido en la transformación al marco rotante.
  \item Se definió una partícula de prueba ubicada a una distancia \texttt{offset} de \(L_1\) (lado solar), con velocidad perpendicular al eje Sol--Júpiter. La magnitud de la velocidad se obtuvo despejando \(v\) de \(C_J=2\,\Omega(\vec r)-v^2\), usando como objetivo el mínimo entre \(C_J^{(\text{Hilda})}\) y un valor ligeramente inferior a \(C_J^{(L_1)}\) (parámetro \texttt{margen}).
  \item Se realizó un barrido de fuerza bruta sobre combinaciones de \texttt{offset} (\(0.005\)--\(0.02\)) y \texttt{margen} (\(0.0001\)--\(0.015\)), integrando cada combinación en ventanas cortas y contando heurísticamente el número de vueltas alrededor de Júpiter (cruces sucesivos del eje \(y=0\) mientras la partícula permanece a menos de \(0.15\) unidades canónicas de Júpiter) antes del escape.
  \item La combinación ganadora se validó integrando a tiempo largo (\(t\in[0,40]\)) para confirmar un escape real y no una órbita cuasi-estable atrapada indefinidamente.
  \item Se verificó la conservación de \(C_J\) a lo largo de la trayectoria final como control de calidad numérico.
\end{enumerate}

\textbf{Nota metodológica.} Este experimento es exploratorio y no constituye un cálculo riguroso de mecánica celeste: el punto \(L_1\) usado es una aproximación de primer orden en \(\mu\) (no la solución exacta de equilibrio del CRTBP); la partícula de prueba \emph{no es} 153 Hilda, sino un cuerpo ficticio que solo toma su \(C_J\) real como referencia energética (físicamente, Hilda se encuentra en resonancia 3:2 con Júpiter, lejos de la región de \(L_1\)); y la búsqueda de parámetros es un barrido tipo \emph{curve-fitting} por fuerza bruta, no una derivación a partir de variedades invariantes de una órbita periódica alrededor de \(L_1\). Por lo tanto, los resultados de esta sección deben interpretarse como una demostración ilustrativa del concepto, no como una predicción física validada.
...
\subsection{Experimento exploratorio: captura balística temporal cerca de \texorpdfstring{\(L_1\)}{L1}}
\label{sec:captura}
Siguiendo el procedimiento descrito en la Sección~\ref{sec:metodologia_captura}, la Tabla~\ref{tab:captura_ref} resume los valores de referencia empleados para construir la condición inicial cerca de \(L_1\), mientras que la Tabla~\ref{tab:captura_resultado} presenta los parámetros óptimos hallados en el barrido y los resultados de la validación a tiempo largo.

\begin{table}[H]
\centering
\caption{Valores de referencia usados para la construcción de la condición inicial cerca de \(L_1\).}
\label{tab:captura_ref}
\begin{tabular}{lr}
\toprule
\textbf{Cantidad} & \textbf{Valor} \\
\midrule
\(C_J\) real de Hilda & 3.02921 \\
\(C_J\) en \(L_1\) (aprox.\ de Euler) & 3.03878 \\
Posición inicial de prueba \((x,y,z)\) & \((0.9208,\ 0,\ 0)\) \\
Velocidad inicial de prueba \((v_x,v_y,v_z)\) & \((0,\ 0.1037,\ 0)\) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Parámetros óptimos del barrido y resultados de la validación a tiempo largo.}
\label{tab:captura_resultado}
\begin{tabular}{lr}
\toprule
\textbf{Parámetro / Resultado} & \textbf{Valor} \\
\midrule
\texttt{offset} óptimo & 0.005 \\
\texttt{margen} óptimo (respecto a \(C_J^{(L_1)}\)) & 0.015 \\
\(C_J\) objetivo final & 3.02378 \\
Vueltas alrededor de Júpiter & 7 \\
Tiempo de escape & \(t\approx13.02\) (unid.\ canónicas) \\
Rango en \(x\) alcanzado & \(-1.22\) a \(1.26\) \\
Deriva máxima de \(C_J\) (control numérico) & \(2.04\times10^{-5}\) \\
\bottomrule
\end{tabular}
\end{table}

La Figura~\ref{fig:zoom_captura} muestra el detalle de la fase de captura previa al escape confirmado en \(t\approx13.02\).

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{imgCRTBP/fig_ZoomFaseDeCaptura.png}
\caption{Fase de captura de la partícula de prueba (energía \(C_J\) inspirada en Hilda) en la vecindad de Júpiter, previa al escape en \(t\approx13\). La trayectoria (cian) realiza varias vueltas alrededor de Júpiter (círculo azul) cerca del punto de Lagrange \(L_1\) (cruz negra), antes de abandonar la región de captura. Resultado de un experimento numérico exploratorio, no una predicción física validada.}
\label{fig:zoom_captura}
\end{figure}

La deriva máxima de \(C_J\) a lo largo de la integración (\(2.04\times10^{-5}\)) es varios órdenes de magnitud menor que el margen energético empleado (0.015), lo que respalda que el número de vueltas y el tiempo de escape reportados no son artefactos numéricos del integrador.

\subsection{Espacios reservados para visualizaciones}
Las figuras restantes aún no se han anexado al proyecto. Por ello, se dejan espacios reservados con nombres de archivo sugeridos para incorporar posteriormente las visualizaciones producidas por el análisis numérico.

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{imgCRTBP/fig_trayectorias_inercial.png}
\caption{Trayectorias de 624 Hektor, 153 Hilda, 617 Patroclus y Lucy en el marco de referencia inercial durante la integración numérica del sistema Sol--Júpiter.}
\label{fig:trayectorias_inercial}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{imgCRTBP/fig_trayectorias_rotantes.png}
\caption{Evolución dinámica de 624 Hektor, 153 Hilda, 617 Patroclus y Lucy en el marco rotante del CRTBP Sol--Júpiter. La visualización permite comparar la libración de los troyanos, la dinámica resonante de Hilda y la trayectoria de la sonda en el potencial efectivo del sistema.}
\label{fig:trayectorias_rotantes}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{imgCRTBP/fig_potencial_modificado.png}
\caption{Mapa del potencial modificado del CRTBP en el plano orbital, con la configuración Sol--Júpiter y la estructura asociada a los puntos de Lagrange.}
\label{fig:potencial_modificado}
\end{figure}
...
El experimento exploratorio de la Sección~\ref{sec:captura} ilustra, de forma cualitativa, cómo una partícula de prueba con energía comparable a la de Hilda puede quedar temporalmente confinada cerca de Júpiter al iniciar su movimiento en la vecindad de \(L_1\), realizando varias vueltas antes de escapar. Este comportamiento es consistente con la existencia de corredores dinámicos de baja energía alrededor de los puntos colineales, ampliamente documentados en la literatura de transferencias de baja energía, aunque el presente análisis no pretende derivarlos rigurosamente.
...
\begin{itemize}
  ...
  \item El experimento de captura cerca de \(L_1\) usa una aproximación de primer orden para la posición de \(L_1\) y un barrido por fuerza bruta, sin recurrir a variedades invariantes ni a una órbita periódica exacta alrededor de \(L_1\); por lo tanto, no debe interpretarse como una predicción física sobre el comportamiento real de 153 Hilda.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Calcular la posición exacta de \(L_1\) y construir órbitas periódicas (por ejemplo, tipo Lyapunov) junto con sus variedades invariantes, para sustentar dinámicamente el fenómeno de captura observado de forma heurística.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item El experimento exploratorio cerca de \(L_1\), con energía inspirada en Hilda, produjo una condición inicial (\texttt{offset}=0.005, \texttt{margen}=0.015) que genera 7 vueltas alrededor de Júpiter antes de escapar en \(t\approx13\), con una deriva de \(C_J\) despreciable, lo que ilustra cualitativamente el fenómeno de captura balística temporal sin constituir una predicción física rigurosa.
\end{enumerate}

\subsection*{Recursos digitales complementarios}
\noindent
Como material complementario, el lector puede consultar las \href{https://github.com/Daniel-534/MecanicaCeleste/tree/main/Modelacion3}{animaciones del sistema Sol--Júpiter en GitHub}, donde se visualiza la evolución temporal de los cuerpos tanto en el marco inercial como en el marco rotante. Además, el desarrollo computacional completo del trabajo está disponible en el \href{https://colab.research.google.com/github/Daniel-534/MecanicaCeleste/blob/main/Modelacion3/CRTBP_Jupiter.ipynb}{notebook interactivo de Google Colab}.
